...
- **Issues & CAPAs** (`/factory/[slug]/actions`) — the accountability loop. A flagged shift entry raises an issue, which then works through four stages — **Open → Investigating → Action taken → Closed**, one at a time, each move paid for with the evidence that stage produces (owner, root cause + corrective action, verification). Enforced by the `actions_stage_transition` trigger, not the UI. One legal skip: an **open** issue can be resolved straight to Closed by a supervisor when it needs no CAPA — it costs a written account of what was done (stored in `verification`) and leaves `investigating_at` null, which is what `resolved_direct` reads to label the text a *resolution* and draw the middle two stages as skipped (`0026`). Overdue and escalated are *computed*, never stored, and there are two clocks: the fix clock stops once the corrective action is in, and a slower sign-off clock (`factories.escalate_hours`) takes over.
...
- **Database** — 26 migrations in `supabase/migrations/`, with RLS on every tenant table.
