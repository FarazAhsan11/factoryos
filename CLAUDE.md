...
- **Database** — 21 migrations in `supabase/migrations/`, with RLS on every tenant table.

Not built: OEE & Downtime, Quality, Trends, handover reports, and the Pipeline's Planning-by-room / Gantt / Archive tabs (they render as "Soon" from `nav.ts`). The shift log's Roster tab was dropped rather than deferred — the shift log has exactly two tabs now.
