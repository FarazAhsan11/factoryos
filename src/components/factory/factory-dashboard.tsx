...
function kpis(unitsPlural: string) {
  return [
    { icon: Gauge, label: "OEE (today)", value: "78.4%", tone: "text-brand" },
    { icon: Boxes, label: "Units produced", value: "12,480", tone: "text-ink" },
    {
      icon: Activity,
    ...
    { icon: Users, label: "On shift", value: "42", tone: "text-ink" },
  ];
}
    ...
    <div className="mx-auto max-w-5xl">
      <div className="flex items-end justify-between gap-4">
        <div>
          ...
          <p className="mt-1 flex items-center gap-1.5 text-sm text-ink-4">
      ...
      {factory.description && (
        <p className="mt-4 max-w-2xl rounded-xl border border-line bg-surface p-4 text-sm text-ink-3">
          {factory.description}
        </p>
      )}
          ...
          {SHIFTS.map((s) => (
            ...
            >
              <div>
                ...
                <p className="text-xs text-ink-5">{s.window}</p>
              </div>
              <span
                className={
                  s.tone === "green"
                    ? "inline-flex items-center rounded-full bg-teal-soft px-2.5 py-0.5 text-xs font-medium text-teal-deep"
                    : "inline-flex items-center rounded-full bg-sunken-2 px-2.5 py-0.5 text-xs font-medium text-ink-3"
                }
              >
                {s.status}
              </span>
